...
export default function Checkout() {
  ...
  const getItemPrice = (planType: string) => {
    const basePrice = 499;
    switch(planType) {
      case 'plus': return basePrice + 100;
      case 'advanced': return basePrice + 199;
  ...
  const subtotal = items.reduce((total: number, item: CheckoutItem) => 
    total + getItemPrice(item.planType) + (item.includeLiveClass ? getLiveClassPrice() * 12 : 0), 0
  );
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="bg-gray-50 dark:bg-gray-800 rounded-3xl p-8">
              <h1 className="text-2xl font-semibold text-gray-900 dark:text-white mb-2">Review Your Bundle</h1>
              <p className="text-gray-600 dark:text-gray-300 mb-8">Grade {grade}</p>

              <div className="space-y-6">
                {items.map((item: CheckoutItem, index: number) => (
                  <div key={index} className="flex gap-6 pb-6 border-b border-gray-200 dark:border-gray-700 last:border-0">
                    <div className="w-16 h-16 bg-white dark:bg-gray-700 rounded-xl flex items-center justify-center flex-shrink-0">
                      <Image
                        ...
                        className="w-16 h-16"
                        onError={(e: any) => {
                          console.error('Image failed to load for subject:', item.subject);
                          e.target.style.display = 'none';
                        }}
                      />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-lg font-medium text-gray-900 dark:text-white">{item.subject}</h3>
                      <div className="text-sm text-gray-600 dark:text-gray-300 space-y-1 mt-1">
                        <div className="text-sm text-gray-500">
                          {item.planType === 'base' ? 'No Add-ons' : 
                           item.planType === 'plus' ? 'Plus Plan' : 
                           'Level 2 Prep'}
          ...
          <div className="lg:w-[380px]">
            <div className="bg-gray-50 dark:bg-gray-800 rounded-3xl p-6">
              ...
              <p className="text-sm text-gray-500 text-center mt-4">
                ...
                <Link href="/terms" className="text-blue-500 hover:text-blue-600">Terms and Conditions</Link>{' '}
                and{' '}
